...
typedef struct fvw_state_t 
  { 
    ...
    bool_t axes;         /* When TRUE, show coordinate axes. */
...
void fvw_paint_coord_axes(float length, double zscale);
  /* Paints coordinate axes {X}, {Y}, {Z} as three line segments
    of the given {length} with origin at {(0,0,0)}.
    The {Z} coordinates will be scaled by {zscale}. */
...
fvw_state_t *fvw_create_state(options_t *o)
  { 
    ...
    w->azimuth = 0;
    w->elevation = 30;
    w->distance = (GLfloat)(sqrt(8)*w->RAD);
...
void fvw_paint_everything(fvw_state_t *w)
  {
    ...
    /* Maint the coordinate axes, if any: */
    if (w->axes)
      { float length = fminf((float)NX_ht, (float)NY_ht);
        if (w->verbose) { fprintf(stderr, "axis length = %15.7e Z-scaled = %15.7f\n", length, ht_scale*length); }
        fvw_paint_coord_axes(length, ht_scale);
      }
    
    /* Paint the reference and {Z=0} planes, if any: */
    if (w->refPlane)
      { double eps = fmax(0.00001*w->RAD, 0.001*ht_vdif);
        float zmin = (float)(ht_scale*ht_vmin - eps);
        float zmax = (float)(ht_scale*ht_vmax + eps);
        if (w->verbose) 
          { fprintf(stderr, "ref planes at orig Z = %15.7e and %15.7e", ht_vmin, ht_vmax);
            fprintf(stderr, "scaled Z = %15.7f and %15.7f\n", zmin, zmax);
          }
...
void fvw_set_perspective(fvw_state_t *w)
  {
    ...
    gluPerspective(angheight, aspect, 0.1, w->distance + sqrt(8)*w->RAD);
...
void fvw_paint_coord_axes(float length, double zscale)
  {
    ...
    float x0 = 0.0f, x1 = x0 + length;
    float y0 = 0.0f, y1 = y0 + length;
    float z0 = 0.0f, z1 = (float)(zscale*(z0 + length));
