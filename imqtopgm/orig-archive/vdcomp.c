...
/*********************************************************************/
   ...
   if (argc == 1);                     /* prompt user for parameters */
                          ...
                          strncmp(argv[1],"?",1)    == 0))
     {
      printf("PDS Image Decompression Program.  Command line format:\n\n");
      printf("VDCOMP [infile] [outfile] [format code]\n");
     ...
     if (output_format == 1)      /* write PDS format blocks */
       {
        if (record_bytes == 836)
          ...
          {
           if ((outfile=fopen(outname,"w",
                              "mrs=1204",FOP,CTX,RECORD_TYPE))==NULL)
             {
              printf("\ncan't open output file: %s\n",outname);
              exit(1);
             }
   ...
   /*****************************************************************/
     {
      ...
      if (record_bytes == 836)
        fwrite("CCSD3ZF0000100000001NJPL3IF0PDS200000001 = SFDU_LABEL",53,1,outfile);
      else
        fwrite("CCSD3ZF0000100000001NJPL3IF0PDS200000001 = SFDU_LABEL",53,1,outfile);      
