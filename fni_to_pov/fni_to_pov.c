...
options_t* parse_options(int argc,char** argv){
  ...
  if(argparser_keyword_present(pp, "-channel")){
    o->channel = (int)argparser_get_next_int(pp,0,10000);
  }
...
float_image_t* scale_mask(float_image_t* mask_img){
  ...
  int WNX = (int)mask_img->sz[1];
  int WNY = (int)mask_img->sz[2];
  ...
  for(x = 0; x < WNX+1; x++){
	...
	fscanf(stdin,"%lf",&w00);
       ...
       double val = 4.0/M;
       float_image_set_sample(cm_img, 0,x,y, (float)val);
     }
  }
...
void generatePOVMesh(
  ...
  options_t* o)
  {
	...
	*/
	/* int NC = (int)fni_map->sz[0]; */
	int NX = (int)fni_map->sz[1];
	int NY = (int)fni_map->sz[2];
...
float_image_t* read_texture_file(char* filename){
  ...
  fclose(arq);
  if ( magic_c== 'P'){
    return float_image_read_pnm_named(filename,FALSE,1,0,TRUE,TRUE,FALSE);
  }else{
...
int main(int argc, char** argv){
  ...
  if(o->viewDist == -1){
    o->viewDist = 5.0*hypot((double)fni_img->sz[1], (double)fni_img->sz[2]);
  }
